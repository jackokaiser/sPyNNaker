                              ...
                              uint32_t neuron_potentials_recording_region_size, uint32_t neuron_gsyns_recording_region_size)
{
  ...
  if (!spike_source_index_block_filled (region_start(1, base_address), flags))
    return (false);
  
  if (!spike_source_spike_vector_filled (region_start(2, base_address), flags))
    return (false);
  
  // Setup output recording regions
  if (!recording_data_filled (base_address, 3, flags, e_recording_channel_spike_history, spike_history_recording_region_size))
    return (false);
